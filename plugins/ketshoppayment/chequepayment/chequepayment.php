...
class plgKetshoppaymentChequepayment extends JPlugin
{
  ...
  public function onKetshopPayment()
  {
    ...
    $query->select('id, name, plugin_element, description')
	  ...
	  ->where('plugin_element='.$db->Quote('chequepayment'))
	  ->where('published=1');
    $db->setQuery($query);
  ...
  /**
   * Builds a payment form allowing the user to pay through a cheque payment or to cancel
   * payment.
   *
   ...
   */
  public function onKetshopPaymentChequepaymentTransaction(): ?string
  {
    $db = JFactory::getDbo();
