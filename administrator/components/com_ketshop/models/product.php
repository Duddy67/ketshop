...
class KetshopModelProduct extends JModelAdmin
{
  ...
  public function getItem($pk = null)
  {
    if($item = parent::getItem($pk)) {
      // Gets both intro_text and full_text together as producttext
  ...
  public function getCategories($pk = null)
  {
    ...
    $query->select('cat_id, catid')
	  ...
	  ->join('LEFT', '#__ketshop_product ON id=product_id')
  ...
  public function getUnallowedCategories($currentCats)
  {
    ...
    foreach ($catids as $i => $catid) {
       ...
       */
      $assetKey = 'com_ketshop.category.'.$oldCat;
  ...
  public function save($data)
  {
    ...
    if(!isset($data['catids'])) {
      $this->setError(JText::_('COM_KETSHOP_DATABASE_ERROR_NO_CATEGORY_SELECTED'));
      return false;
    }
  ...
  public function saveorder($pks = null, $order = null)
  {
    ...
    if(isset($post['filter']['category_id']) && count($post['filter']['category_id']) == 1) {
      ...
      $query->select('product_id, cat_id, ordering')
	    ->from('#__ketshop_product_cat_map')
	    ->where('cat_id='.(int)$catId);
      $db->setQuery($query);
      $items = $db->loadAssocList('product_id');
	...
	elseif($items[$pk]['ordering'] != $order[$i]) {
	  ...
	  $query->clear();
	  $query->update('#__ketshop_product_cat_map')
