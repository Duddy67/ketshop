...
class KetshopModelShipping extends JModelAdmin
{
  ...
  public function validate($form, $data, $group = null)
  {
    ...
    if($data['delivery_type'] == 'at_delivery_point') {
  ...
  public function getItem($pk = null)
  {
    if($item = parent::getItem($pk)) {
      // Truncates some variables.
      $item->min_weight = UtilityHelper::floatFormat($item->min_weight, 2);
      $item->max_weight = UtilityHelper::floatFormat($item->max_weight, 2);
      $item->delivpnt_cost = UtilityHelper::floatFormat($item->delivpnt_cost, 2);
    }
  ...
  /**
   * Returns the delivery point addresses for a given shipping.
   *
   * @param   integer  $pk  The id of the primary key.
   *
   * @return  array         The delivery point addresses.
   */
  ...
  /**
   * Returns the destination data for a given shipping.
   *
   * @param   integer  $pk  The id of the primary key.
   *
   * @return  array         The destination data.
   */
