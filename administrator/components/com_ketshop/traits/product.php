...
trait ProductTrait
{
  ...
  /**
   * Returns the image data bound to a given product.  
   *
   * @param   integer  $productId  The id of the product.
   * @param   boolean  $isAdmin    Flag which indicates whether the data are fetched from
   *                               the administrator side or from the site side.
   *
   * @return  array    An array of image data.
   */
  public function getImageData($productId, $isAdmin = false) 
  {
    $db = JFactory::getDbo();
    $query = $db->getQuery(true);
    $query->select('src, width, height, alt, variant_code, ordering, src AS image_path') 
    ...
    if($isAdmin) {
      // Appends "../" to the path of each image as we are in the administrator area.
  ...
  public function getVariantData($productId) 
  {
    ...
    $query->select('prod_id, var_id, var_id AS id_nb, name, TRUNCATE(base_price,2) AS base_price, TRUNCATE(price_with_tax,2) AS price_with_tax,'.
  ...
  public function getAttribute($attributeId) 
  {
    ...
    foreach($options as $key => $option) {
      // Appends empty selected value to each option as no value has been selected yet.
  ...
  /**
   * Returns the id of the categories bound to a given product.
   *
   * @param   integer  $productId  The id of the product.
   *
