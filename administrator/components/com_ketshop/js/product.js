(function($) {
  ...
  const items = {'attribute':3, 'image':[6,1], 'variant':[8,6,6,1]};
  ...
  validateFields = function(e) {
	...
	fields = {'base_price':'unsigned_float', 'min_stock_threshold':'unsigned_int', 'max_stock_threshold':'unsigned_int', 'min_quantity':'unsigned_int', 'max_quantity':'unsigned_int'};
  ...
  populateImageItem = function(idNb, data) {
    // Defines the default field values.
    if(data === undefined) {
      data = {'src':'', 'width':'', 'height':'', 'alt':'', 'variant_code':'', 'image_path':''};
    }
    ...
    attribs = {'type':'hidden', 'name':'image_image_src_'+idNb, 'id':'image-image-src-'+idNb, 'value':data.src};
    let elem = GETTER.image.createElement('input', attribs);
    $('#image-row-1-cell-1-'+idNb).append(elem);
    attribs = {'type':'hidden', 'name':'image_image_width_'+idNb, 'id':'image-image-width-'+idNb, 'value':data.width};
    elem = GETTER.image.createElement('input', attribs);
    $('#image-row-1-cell-1-'+idNb).append(elem);
    attribs = {'type':'hidden', 'name':'image_image_height_'+idNb, 'id':'image-image-height-'+idNb, 'value':data.height};
    elem = GETTER.image.createElement('input', attribs);
    $('#image-row-1-cell-1-'+idNb).append(elem);
    ...
    // Creates url to the com_media component. Both the itemType and id number are
    // concatenated in the fieldid value.
    let url = $('#root-location').val()+'administrator/index.php?option=com_media&view=images&tmpl=component&asset=com_ketshop&author='+userId+'&fieldid=image-'+idNb+'&folder=ketshop';
    let button = GETTER.image.createButton('select', idNb, url);
    $('#image-row-1-cell-1-'+idNb).append(button);

    // Element label.
    attribs = {'title':Joomla.JText._('COM_KETSHOP_IMAGE_TITLE'), 'class':'item-label', 'id':'image-imagepath-label-'+idNb};
    $('#image-row-1-cell-2-'+idNb).append(GETTER.image.createElement('span', attribs));
    $('#image-imagepath-label-'+idNb).text(Joomla.JText._('COM_KETSHOP_IMAGE_LABEL'));

    attribs = {'type':'text', 'disabled':'disabled', 'id':'image-image-path-'+idNb, 'class':'image-path', 'value':data.image_path};
    $('#image-row-1-cell-2-'+idNb).append(GETTER.image.createElement('input', attribs));

    attribs = {'class':'btn preview-btn', 'id':'image-image-preview-'+idNb};
    $('#image-row-1-cell-2-'+idNb).append(GETTER.image.createElement('span', attribs));
    attribs = {'class':'icon-eye'};
    $('#image-image-preview-'+idNb).append(GETTER.image.createElement('span', attribs));

    // Uses the JQuery popover functionality.
    $('#image-image-preview-'+idNb).popover({
      html: true,
      trigger: 'hover',
      content: function () {
        return '<img src="'+$('#image-image-src-'+idNb).val()+'" />';
     }
    });

    // Alt label.
    attribs = {'title':Joomla.JText._('COM_KETSHOP_IMAGE_ALT_TITLE'), 'class':'item-label', 'id':'image-alt-label-'+idNb};
    $('#image-row-1-cell-3-'+idNb).append(GETTER.image.createElement('span', attribs));
    $('#image-alt-label-'+idNb).text(Joomla.JText._('COM_KETSHOP_IMAGE_ALT_LABEL'));

    // Alt text input.
    attribs = {'type':'text', 'name':'image_image_alt_'+idNb, 'id':'image-image-alt-'+idNb, 'value':data.alt};
    $('#image-row-1-cell-3-'+idNb).append(GETTER.image.createElement('input', attribs));

    // Variant code label.
    attribs = {'title':Joomla.JText._('COM_KETSHOP_IMAGE_VARIANT_CODE_TITLE'), 'class':'item-label', 'id':'image-variant-code-label-'+idNb};
    $('#image-row-1-cell-4-'+idNb).append(GETTER.image.createElement('span', attribs));
    $('#image-variant-code-label-'+idNb).text(Joomla.JText._('COM_KETSHOP_IMAGE_VARIANT_CODE_LABEL'));

    // Variant code text input.
    attribs = {'type':'text', 'name':'image_image_variant_code_'+idNb, 'class':'item-small-field', 'id':'image-image-variant-code-'+idNb, 'value':data.variant_code};
    $('#image-row-1-cell-4-'+idNb).append(GETTER.image.createElement('input', attribs));
  }
  ...
  populateVariantItem = function(idNb, data) {
    // Defines the default field values.
    if(data === undefined) {
      data = {'id_nb':idNb, 'name':'', 'base_price':'', 'price_with_tax':'', 'stock':'', 'sales':'', 'published':0, 'stock_subtract':1, 'allow_order':1, 'min_stock_threshold':5, 'max_stock_threshold':20, 'min_quantity':1, 'max_quantity':20, 'weight':'', 'length':'', 'width':'', 'height':'', 'code':'', 'availability_delay':'', 'attributes':[]};

      if($('#jform_id').val() != 0) {
	data = setDefaultVariantData(data);
      }
    }
	...
	if(data.published == 1) {
      ...
      if(key == 'sales' || key == 'price_with_tax' || (productType == 'bundle' && (key == 'stock' || key == 'availability_delay'))) {
  ...
  setDefaultVariantData = function(data) {
    // Gets the id number of the basic variant (ie: the first variant in the list).
    let basicIdNb = GETTER.variant.idNbList[0];
    // The variant attributes to set.
    let varAttribs = ['base_price','price_with_tax','stock_subtract','allow_order','min_stock_threshold','max_stock_threshold','min_quantity','max_quantity','weight','length','width','height'];


    for(let i = 0; i < varAttribs.length; i++) {
      data[varAttribs[i]] = $('#variant-'+varAttribs[i]+'-'+basicIdNb).val();
    }
  ...
  selectAttributeItem = function(id, name, idNb, dynamicItemType) {
    ...
    // Checks for a previously selected attribute.
    if($('#attribute-attribute-id-'+idNb).val() != '') {
      // Removes the previously selected attribute from the variants.
      beforeRemoveItem(idNb, dynamicItemType);
    }
