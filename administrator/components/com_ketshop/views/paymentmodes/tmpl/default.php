...
$saveOrder = $listOrder == 'pm.ordering';
 
if($saveOrder) {
  $saveOrderingUrl = 'index.php?option=com_ketshop&task=taxes.saveOrderAjax&tmpl=component';
  JHtml::_('sortablelist.sortable', 'paymentmodeList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
?>


<form action="<?php echo JRoute::_('index.php?option=com_ketshop&view=paymentmodes');?>" method="post" name="adminForm" id="adminForm">
...
?>
  ...
  <?php else : ?>
	...
	<th width="1%" class="nowrap center hidden-phone">
	  <?php echo JHtml::_('searchtools.sort', '', 'pm.ordering', $listDirn, $listOrder, null, 'asc', 'JGRID_HEADING_ORDERING', 'icon-menu-2'); ?>
	</th>
	<th width="1%" class="hidden-phone">
	  <?php echo JHtml::_('grid.checkall'); ?>
	</th>
	<th width="1%" style="min-width:55px" class="nowrap center">
	  <?php echo JHtml::_('searchtools.sort', 'JSTATUS', 'pm.published', $listDirn, $listOrder); ?>
	</th>
	<th>
	...
	<th width="1%" class="nowrap hidden-phone">
	  <?php echo JHtml::_('searchtools.sort', 'JGRID_HEADING_ID', 'pm.id', $listDirn, $listOrder); ?>
	</th>
      </tr>
      </thead>
      ...
      $ordering = ($listOrder == 'pm.ordering');
      $canEdit = $user->authorise('core.edit','com_ketshop.paymentmode.'.$item->id);
      $canEditOwn = $user->authorise('core.edit.own', 'com_ketshop.paymentmode.'.$item->id) && $item->created_by == $userId;
      $canCheckin = $user->authorise('core.manage','com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
      $canChange = ($user->authorise('core.edit.state','com_ketshop.paymentmode.'.$item->id) && $canCheckin) || $canEditOwn; 
      ?>
	  ...
	  <td class="has-context">
		...
		<a href="<?php echo JRoute::_('index.php?option=com_ketshop&task=paymentmode.edit&id='.$item->id);?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
