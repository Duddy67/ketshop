...
// Check only against component permission as currency items have no categories.
$canOrder = $user->authorise('core.edit.state', 'com_ketshop');
?>


<form action="<?php echo JRoute::_('index.php?option=com_ketshop&view=currencies');?>" method="post" name="adminForm" id="adminForm">
...
?>
  ...
  <?php else : ?>
	...
	<th width="5%">
	  <?php echo JHtml::_('searchtools.sort', 'COM_KETSHOP_HEADING_ALPHA', 'c.alpha', $listDirn, $listOrder); ?>
	</th>
	...
	<th width="1%" class="nowrap hidden-phone">
      ...
      $canEdit = $user->authorise('core.edit','com_ketshop.currency.'.$item->id);
      $canEditOwn = $user->authorise('core.edit.own', 'com_ketshop.currency.'.$item->id) && $item->created_by == $userId;
      $canCheckin = $user->authorise('core.manage','com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
      $canChange = ($user->authorise('core.edit.state','com_ketshop.currency.'.$item->id) && $canCheckin) || $canEditOwn; 
      ?>
	  ...
	  <td class="has-context">
		...
		<a href="<?php echo JRoute::_('index.php?option=com_ketshop&task=currency.edit&id='.$item->id);?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
