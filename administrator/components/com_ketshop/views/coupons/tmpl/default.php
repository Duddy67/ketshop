...
<form action="<?php echo JRoute::_('index.php?option=com_ketshop&view=coupons');?>" method="post" name="adminForm" id="adminForm">
...
?>
  ...
  <?php else : ?>
	...
	<th width="10%">
	  <?php echo JHtml::_('searchtools.sort', 'COM_KETSHOP_HEADING_CODE', 'c.code', $listDirn, $listOrder); ?>
	</th>
	<th width="20%">
	  <?php echo JHtml::_('searchtools.sort', 'COM_KETSHOP_HEADING_PRICERULE', 'prule_name', $listDirn, $listOrder); ?>
	</th>
	...
	<th width="1%" class="nowrap hidden-phone">
      ...
      $canEdit = $user->authorise('core.edit','com_ketshop.coupon.'.$item->id);
      $canEditOwn = $user->authorise('core.edit.own', 'com_ketshop.coupon.'.$item->id) && $item->created_by == $userId;
      $canCheckin = $user->authorise('core.manage','com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
      $canChange = ($user->authorise('core.edit.state','com_ketshop.coupon.'.$item->id) && $canCheckin) || $canEditOwn; 
      ?>
	  ...
	  <td class="has-context">
		...
		<a href="<?php echo JRoute::_('index.php?option=com_ketshop&task=coupon.edit&id='.$item->id);?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
