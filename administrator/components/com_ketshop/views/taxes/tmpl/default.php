...
// Check only against component permission as tax items have no categories.
...
<form action="<?php echo JRoute::_('index.php?option=com_ketshop&view=taxes');?>" method="post" name="adminForm" id="adminForm">
...
?>
  ...
  <?php else : ?>
    <table class="table table-striped" id="taxList">
      <thead>
      <tr>
	...
	<th width="10%">
	  <?php echo JHtml::_('searchtools.sort', 'COM_KETSHOP_HEADING_RATE', 't.rate', $listDirn, $listOrder); ?>
	</th>
	...
	<th width="1%" class="nowrap hidden-phone">
      ...
      $canEdit = $user->authorise('core.edit','com_ketshop.tax.'.$item->id);
      $canEditOwn = $user->authorise('core.edit.own', 'com_ketshop.tax.'.$item->id) && $item->created_by == $userId;
      $canCheckin = $user->authorise('core.manage','com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
      $canChange = ($user->authorise('core.edit.state','com_ketshop.tax.'.$item->id) && $canCheckin) || $canEditOwn; 
      ?>
	  ...
	  <td class="center">
	    <div class="btn-group">
	      <?php echo JHtml::_('jgrid.published', $item->published, $i, 'taxes.', $canChange, 'cb'); ?>
	      <?php
	      // Create dropdown items
	      $action = $archived ? 'unarchive' : 'archive';
	      JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'taxes');
	  ...
	  <td class="has-context">
		...
		<a href="<?php echo JRoute::_('index.php?option=com_ketshop&task=tax.edit&id='.$item->id);?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
	  ...
	  <td>
	    <?php echo $this->escape($item->rate).JText::_(' %'); ?>
	  </td>
