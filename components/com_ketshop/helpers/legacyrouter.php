...
/**
 * Legacy routing rules class from com_ketshop
 *
 * @since       3.6
 * @deprecated  4.0
 */
class KetshopRouterRulesLegacy implements JComponentRouterRulesInterface
{
  /**
  ...
  /**
   * Preprocess the route for the com_ketshop component
   *
  ...
  public function build(&$query, &$segments)
  {
    ...
    // Check again
    if($menuItemGiven && isset($menuItem) && $menuItem->component != 'com_ketshop') {
      $menuItemGiven = false;
      unset($query['Itemid']);
    }
    ...
    // Are we dealing with a product or a category that is attached to a menu item?
    ...
    $views = array('category', 'product', 'cart', 'connection', 'checkout', 'payment', 'orders');
   ...
   */
    ...
    if(!$category) {
      JError::raiseError(404, JText::_('COM_KETSHOP_ERROR_PARENT_CATEGORY_NOT_FOUND'));
      return;
    }
