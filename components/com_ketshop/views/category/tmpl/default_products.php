...
<table class="table category table-striped">
  ...
  <tbody>
	...
	<?php  // Build the link to the login page for the user to login or register.
		...
		$returnURL = JRoute::_(KetshopHelperRoute::getProductRoute($item->slug, $this->state->get('category.id'), $item->language));
		$link = new JUri($link1);
		$link->setVar('return', base64_encode($returnURL));
	      endif; ?>

	<?php if($item->params->get('access-view')) : // Set the link to the product page.
	      $link = JRoute::_(KetshopHelperRoute::getProductRoute($item->slug, $this->state->get('category.id'), $item->language));
	  endif; ?>
