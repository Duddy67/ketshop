...
foreach($tabs as $key => $tab) {
  ...
  $rendering = JLayoutHelper::render('product.'.$key, array('product' => $product, 'variant' => $variant, 'params' => $params));
...
<?php foreach($tabs as $key => $tab) : ?>
    <li <?php echo $active; ?>>
    <a data-toggle="tab" href="#<?php echo $key.'-'.$variant->prod_id.'-'.$variant->var_id; ?>"><?php echo JText::_('COM_KETSHOP_PRODUCT_'.strtoupper($key)); ?></a></li>
<?php
...
<?php foreach($tabs as $key => $tab) : ?>
    <div id="<?php echo $key.'-'.$variant->prod_id.'-'.$variant->var_id; ?>" class="tab-pane fade<?php echo $active; ?>">
    <?php echo $tab; ?>
    </div>
<?php
